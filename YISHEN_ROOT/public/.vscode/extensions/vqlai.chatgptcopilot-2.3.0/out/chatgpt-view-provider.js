...
class ChatGptViewProvider {
    ...
    async resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(data => {
            ...
            if (data.type === 'vsCodeToken') {
                this.context.globalState.update("ChatGPTCopilot.vsCodeToken", data.detail);
            }
            if (data.type === 'vsCodeCleanToken') {
                this.context.globalState.update("ChatGPTCopilot.vsCodeToken", "");
            }
        });
    }
    ...
    async selectSearch(userInput, textCode) {
        // 如果是首次，插件窗口强制弹出
        if (!this.webView) {
            await vscode.commands.executeCommand('ChatGPTCopilot-vscode-plugin.view.focus');
            setTimeout(() => {
    ...
    async getHtml(webview) {
        ...
        if (webviewUrl === "") {
            return "啊哦~您当前访问 V起来·ChatGPTCopilot 的网络不畅，" +
        ...
        const vsCodeToken = this.context.globalState.get("ChatGPTCopilot.vsCodeToken") ?? "";
        const extension = vscode.extensions.getExtension("vqilai.ChatGPTCopilot");
        ...
        <html lang="en">
            ...
            <input id="inputCV" type="hidden"></input>
                <iframe id='ChatGPTCopilotIframe' class="iframeDiv" src="${webviewUrl}&time=${time}" scrolling="none" frameborder="0" 
            ...
            <script>
                ...
                var ChatGPTCopilotIframe = document.getElementById('ChatGPTCopilotIframe');    

                function sendTokenToIframe(){
                    var data = {};
                    data.eventType = "vsCodeToken";
                    data.detail = "${vsCodeToken}";
                    ChatGPTCopilotIframe.contentWindow.postMessage(data, '*');
                }
                        ...
                        if(${IS_OSX}){
                            ...
                            window.navigator.clipboard.readText().then(
                                clipText => {
                                    ...
                                    ChatGPTCopilotIframe.contentWindow.postMessage(postData, '*');
                                }
                            );
                        }
                    }
