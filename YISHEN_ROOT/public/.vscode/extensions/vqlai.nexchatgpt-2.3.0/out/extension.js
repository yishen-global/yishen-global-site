...
function activate(context) {
    ...
    console.log('Congratulations, your extension "NexChatGPT" is now active!');
    const chatViewProvider = new chatgpt_view_provider_1.ChatGptViewProvider(context);
    let webviewProvider = vscode.window.registerWebviewViewProvider("NexChatGPT-vscode-plugin.view", chatViewProvider, {
        webviewOptions: { retainContextWhenHidden: true }
    });
    const explainCodeRegister = vscode.commands.registerCommand('NexChatGPT.explainCode', explainCode);
    const addCommentRegister = vscode.commands.registerCommand('NexChatGPT.addComment', addComment);
    const addTestCaseRegister = vscode.commands.registerCommand('NexChatGPT.addTestCase', addTestCase);
    const addUnitTestRegister = vscode.commands.registerCommand('NexChatGPT.addUnitTest', addUnitTest);
    const improveCodeRegister = vscode.commands.registerCommand('NexChatGPT.improveCode', improveCode);
