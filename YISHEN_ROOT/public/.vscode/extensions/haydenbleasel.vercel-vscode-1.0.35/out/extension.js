...
// src/utils/fetchDeployments.ts
var import_cross_fetch = __toESM(require_node_ponyfill());
var fetchDeployments = async (accessToken, projectId, teamId) => {
  const response = await (0, import_cross_fetch.default)(
    teamId ? `https://api.vercel.com/v6/deployments?teamId=${teamId}&projectId=${projectId}&limit=1` : `https://api.vercel.com/v6/deployments?projectId=${projectId}&limit=1`,
    {
      method: "GET",
      headers: {
...
var fetchDeployments_default = fetchDeployments;
...
// src/utils/config.ts
var vercelSchema = z.object({
  projectId: z.string(),
  orgId: z.string()
});
var getProjectPaths = async () => await import_vscode.workspace.findFiles("**/.vercel/project.json", "**/node_modules/**");
var getProjectFiles = async (paths) => Promise.all(
  paths.map(async (path) => {
    const file = await import_vscode.workspace.fs.readFile(path);
...
var getDeploymentStatus = async (projectFile, accessToken) => {
  try {
    const deployments = await fetchDeployments_default(
      accessToken,
    ...
    const formattedDate = createdAt ? formatDistance2(new Date(createdAt), /* @__PURE__ */ new Date()) : "a while";
    return [
      name ?? "unknown repo",
      `(${state.toLowerCase()})`,
      `${formattedDate} ago`,
    ...
    return parseError_default(error);
  }
};
var getAccessToken = () => import_vscode.workspace.getConfiguration("vercelVSCode").get("accessToken");

// src/utils/const.ts
var triangle = "$(debug-breakpoint-function-unverified)";
...
var interval = null;
var activate = async () => {
  const statusBarItem = import_vscode2.window.createStatusBarItem(
    import_vscode2.StatusBarAlignment.Right,
    100
  );
  ...
  if (!projectPaths.length) {
    ...
    statusBarItem.tooltip = "No Vercel projects found. Please run `vercel link` in your project directory to link your project to Vercel.";
    return;
  }
  ...
  if (!accessToken) {
    statusBarItem.text = `${triangle} Error`;
    statusBarItem.tooltip = "Vercel access token not found. Please set your Vercel Access Token in the extension settings.";
    return;
  }
  ...
  const update = async () => {
    try {
      ...
      statusBarItem.text = `${triangle} Error`;
      statusBarItem.tooltip = parseError_default(error);
    }
  };
  await update();
