...
function activate(context) {
    ...
    console.log('Congratulations, your extension "ChatGPT4Chinese" is now active!');
    const chatViewProvider = new chatgpt_view_provider_1.ChatGptViewProvider(context);
    let webviewProvider = vscode.window.registerWebviewViewProvider("ChatGPT-4-Chinese-vscode-plugin.view", chatViewProvider, {
        webviewOptions: { retainContextWhenHidden: true }
    });
    const explainCodeRegister = vscode.commands.registerCommand('ChatGPT4Chinese.explainCode', explainCode);
    const addCommentRegister = vscode.commands.registerCommand('ChatGPT4Chinese.addComment', addComment);
    const addTestCaseRegister = vscode.commands.registerCommand('ChatGPT4Chinese.addTestCase', addTestCase);
    const addUnitTestRegister = vscode.commands.registerCommand('ChatGPT4Chinese.addUnitTest', addUnitTest);
    const improveCodeRegister = vscode.commands.registerCommand('ChatGPT4Chinese.improveCode', improveCode);
