...
class ChatGptViewProvider {
    ...
    async resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(data => {
            ...
            if (data.type === 'vsCodeToken') {
                this.context.globalState.update("MiniGPT.vsCodeToken", data.detail);
            }
            if (data.type === 'vsCodeCleanToken') {
                this.context.globalState.update("MiniGPT.vsCodeToken", "");
            }
        });
    }
    ...
    async selectSearch(userInput, textCode) {
        // 如果是首次，插件窗口强制弹出
        if (!this.webView) {
            await vscode.commands.executeCommand('MiniGPT-vscode-plugin.view.focus');
            setTimeout(() => {
    ...
    async getHtml(webview) {
        ...
        if (webviewUrl === "") {
            return "啊哦~您当前访问 V起来·MiniGPT 的网络不畅，" +
        ...
        const vsCodeToken = this.context.globalState.get("MiniGPT.vsCodeToken") ?? "";
        const extension = vscode.extensions.getExtension("vqilai.MiniGPT");
        ...
        <html lang="en">
            ...
            <input id="inputCV" type="hidden"></input>
                <iframe id='MiniGPTIframe' class="iframeDiv" src="${webviewUrl}&time=${time}" scrolling="none" frameborder="0" 
            ...
            <script>
                ...
                var MiniGPTIframe = document.getElementById('MiniGPTIframe');    

                function sendTokenToIframe(){
                    var data = {};
                    data.eventType = "vsCodeToken";
                    data.detail = "${vsCodeToken}";
                    MiniGPTIframe.contentWindow.postMessage(data, '*');
                }
                        ...
                        if(${IS_OSX}){
                            ...
                            window.navigator.clipboard.readText().then(
                                clipText => {
                                    ...
                                    MiniGPTIframe.contentWindow.postMessage(postData, '*');
                                }
                            );
                        }
                    }
