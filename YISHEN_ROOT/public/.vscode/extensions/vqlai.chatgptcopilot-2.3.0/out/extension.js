...
function activate(context) {
    ...
    console.log('Congratulations, your extension "ChatGPTCopilot" is now active!');
    const chatViewProvider = new chatgpt_view_provider_1.ChatGptViewProvider(context);
    let webviewProvider = vscode.window.registerWebviewViewProvider("ChatGPTCopilot-vscode-plugin.view", chatViewProvider, {
        webviewOptions: { retainContextWhenHidden: true }
    });
    const explainCodeRegister = vscode.commands.registerCommand('ChatGPTCopilot.explainCode', explainCode);
    const addCommentRegister = vscode.commands.registerCommand('ChatGPTCopilot.addComment', addComment);
    const addTestCaseRegister = vscode.commands.registerCommand('ChatGPTCopilot.addTestCase', addTestCase);
    const addUnitTestRegister = vscode.commands.registerCommand('ChatGPTCopilot.addUnitTest', addUnitTest);
    const improveCodeRegister = vscode.commands.registerCommand('ChatGPTCopilot.improveCode', improveCode);
